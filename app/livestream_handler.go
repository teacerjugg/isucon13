...
func getLivestreamTags(ctx context.Context, tx *sqlx.Tx, livestreamID int64) ([]Tag, error) {
	...
	livestreamTags := make([]Tag, len(livestreamTagModels))
	for i := range livestreamTagModels {
		tagIdx := slices.IndexFunc(tags, func(tag *Tag) bool { return tag.ID == livestreamTagModels[i].TagID })
		if tagIdx == -1 {
			return nil, errors.New("tagIdx == -1")
		}
		tag := tags[tagIdx]

		livestreamTags[i] = Tag{
			ID:   tag.ID,
			Name: tag.Name,
		}
	}
...
func fillLivestreamResponse(ctx context.Context, tx *sqlx.Tx, livestreamModel LivestreamModel) (Livestream, error) {
	...
	livestreamTags, err := getLivestreamTags(ctx, tx, livestreamModel.ID)
	if err != nil {
		return Livestream{}, err
	}
